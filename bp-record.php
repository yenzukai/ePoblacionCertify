...
<?php
    ...
    $business_name = $mysqli->real_escape_string($_POST['business_name']);
    $business_number = $mysqli->real_escape_string($_POST['business_number']);
    ...
    if (isset($_FILES['support_doc']) && $_FILES['support_doc']['error'] == 0) {
        ...
        // Validate file type (you may need more checks based on your requirements)
    ...
    $query = "INSERT INTO requester (requester_id, first_name, middle_name, last_name, house_no, street_name, barangay, municipality, province, zip_code, email, phone_number, certificate_type, business_name, business_number, pick_up_date, payment_method, reference_number, purpose, support_doc)
        VALUES ('$tracking_number', '$first_name', '$middle_name', '$last_name', '$house_no', '$street_name', '$barangay', '$municipality', '$province', '$zip_code', '$email', '$phone_number', '$certificate_type', '$business_name', '$business_number', '$pick_up_date', '$payment_method', '$reference_number', '$purpose', '$support_doc')";
    ...
    if ($mysqli->query($query) === TRUE) {
        ...
        <div class="confirm-tab">
            ...
            <a href="bp-cert-gen.php" target="_blank">Click to download your certificate now!<br /></a>
