...
<body>
    ...
    if ($result->num_rows > 0) {
        ...
        echo generateCertificate($row['first_name'], $row['middle_name'], $row['last_name'], $row['house_no'], $row['street_name'], $row['barangay'], $row['municipality'], $row['province'], $row['zip_code'], $row['purpose'], $row['date_requested']);
        echo "<br><br>";
    } 
    else {
    ...
    function generateCertificate($first_name, $middle_name, $last_name, $house_no, $street_name, $barangay, $municipality, $province, $zip_code, $purpose, $date_requested)
    {
    ...
    // You can customize the certificate template as needed
        ...
        img.signature {
            position: absolute;
            top: 760px; /* Adjust the top position as needed */
            left: 215px; /* Adjust the left position as needed */
            max-height: 150px; /* Adjust the max-height as needed */
        }
        ...
        <p>This is to certify that <strong>$first_name $middle_name $last_name</strong> is a resident of <strong>$house_no $street_name, Brgy. $barangay $municipality $province $zip_code</strong> and has no derogatory record filed in our Barangay Office</p>
        <p>The above-named individual who is a bonafide resident of this barangay is a person of good moral character, peace-loving and civic minded citizen.</p>
        <p>This certification/clearance is hereby issued in connection with the subject's application for <strong>$purpose</strong> and for whatever legal purpose it may serve him/her best, and is valid for six (6) months from the date issued.</p>
